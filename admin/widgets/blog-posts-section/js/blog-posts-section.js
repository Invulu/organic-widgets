...
jQuery(document).ready(function($){
	...
	blogPostsSectionWidgetImage = {
		...
		uploader : function( widget_id, widget_id_string ) {
			...
			var frame = wp.media({
				title : blogPostsSectionWidget.frame_title,
				multiple : false,
				library : { type : 'image' },
				button : { text : blogPostsSectionWidget.button_title }
			});
			...
			frame.on('close',function( ) {
				...
				blogPostsSectionWidgetImage.render( widget_id, widget_id_string, attachments[0] );
			});
		...
		render : function( widget_id, widget_id_string, attachment ) {
			...
			$("#" + widget_id_string + 'preview').html(blogPostsSectionWidgetImage.imgHTML( attachment ));
		...
		remover : function( widget_id, widget_id_string, field_name ) {
			...
			blogPostsSectionWidgetImage.render( widget_id, widget_id_string, false );
